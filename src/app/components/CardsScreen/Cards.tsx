...
const Projects: React.FC = () => {
  ...
  return (
    <section
      id="projects"
      className={`min-h-screen flex flex-col items-center justify-start px-6 py-20 transition-colors duration-500 ${sectionClasses}`}
    >
            ...
            <div className="p-6">
              ...
              <div className="flex flex-wrap gap-2 mt-4">
                {project.tags.map((tag) => (
                  <span key={tag} className={`px-3 py-1 text-xs rounded-full ${tagClasses}`}>
                    {tag}
                  </span>
                ))}
              </div>
